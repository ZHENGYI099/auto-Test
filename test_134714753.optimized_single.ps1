...
Write-Host "Verification: Manual check required" -ForegroundColor Yellow
...
try {
    $verifyExitCode=1
    $s=Get-WmiObject -Class Win32_Service -Filter "Name='cmdextension'"; if ($s -and $s.State -eq 'Running' -and $s.StartMode -eq 'Auto' -and $s.DelayedAutoStart -eq $true -and $s.StartName -eq 'LocalSystem') {$verifyExitCode=0} else {$verifyExitCode=1}
    if ($verifyExitCode -eq 0) {
        Write-Host "    [OK] Verification passed" -ForegroundColor Green
